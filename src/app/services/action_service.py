...
async def complete_action(
    ...
    if action.status not in (ActionStatus.PENDING, ActionStatus.IN_PROGRESS):
        raise InvalidActionStateError(f"Cannot complete action in {action.status} status.")
...
async def skip_action(
    ...
    if action.status not in (ActionStatus.PENDING, ActionStatus.IN_PROGRESS):
        raise InvalidActionStateError(f"Cannot skip action in {action.status} status.")
