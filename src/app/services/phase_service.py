...
from app.constants.goals import GoalStatus, PhaseStatus
...
async def create_phase(
    ...
    await publish_phase_event(
        event_type=PhaseEvents.UNLOCKED if initial_status == PhaseStatus.ACTIVE else "phase.created",
...
async def complete_phase(user_id: str, phase_id: str) -> Phase:
    ...
    if phase.status not in (PhaseStatus.ACTIVE, PhaseStatus.LOCKED):
        raise InvalidPhaseStateError(
            f"Cannot complete phase in {phase.status} status."
        )
...
async def skip_phase(user_id: str, phase_id: str, reason: Optional[str] = None) -> Phase:
    ...
    if phase.status not in (PhaseStatus.ACTIVE, PhaseStatus.LOCKED):
        raise InvalidPhaseStateError(
            f"Cannot skip phase in {phase.status} status."
        )
